...
4. **Ugeplan**: *V1 (bygget)* – 7 dage × måltider (morgen, middag, aften), hver et dropdown med opskrifter, der passer til barnets alder (fra fødselsdatoen i Indstillinger). Gemmes lokalt.
   - **Indstillinger (fødselsdato)**: *V1 (bygget, minimal)* – kun fødselsdato indtil videre (`indstillinger.js`), bruges til at beregne alder i måneder og styre, hvilke opskrifter der kan vælges i Ugeplan.
   - **"Foreslå uge"**: fylder alle 21 felter automatisk. Morgen foretrækker grød, middag foretrækker grøntsagsmos/middag/familiemad, aften foretrækker middag/familiemad/pålæg/grøntsagsmos. Fra 6 måneder sikrer den aktivt mindst én kød/fisk-opskrift pr. dag (tvinger sidste måltid til at være kød/fisk, hvis dagen ellers ville mangle det). Bruger mindst-brugt-først for variation henover ugen.
   - **Regeltjek** (kører løbende, også ved manuel valg): under 4 mdr → besked om at ugeplan ikke er relevant endnu; fra 6 mdr advares hvis en dag mangler kød/fisk; under 6 mdr adviseres hvis der er mere end 2 grødmåltider på en dag; advarer hvis frugtmos er sat som et helt måltid i stedet for tilbehør; advarer hvis en valgt opskrift reelt er for gammel til barnets alder; advarer hvis ris/risgrød optræder mere end 3 gange på ugen; minder blidt om ca. ½ hårdkogt æg et par gange om ugen, hvis ugeplanen slet ikke indeholder æg.
5. **Indkøbsliste og mealprep**: Saml ingredienser for ugen, skalér efter antal portioner, træk det fra, vi har i køleskabet, og gruppér efter butiksafdeling. Lav en prep-plan: hvad der koges samme dag, hvad der fryses i portioner, og hvornår det skal bruges.
   - **"Tilføj til indkøbsliste" fra en opskrift**: *V1 (bygget)* – på opskriftssiden vælger man først ønsket antal portioner (skalerer automatisk alle mængder op/ned ud fra opskriftens standardportioner), og trykker så på knappen. Vand tilføjes aldrig. Ved mælk-valg (modermælk/modermælkserstatning) tilføjes kun modermælkserstatning.
     - **Indkøbsomregning** (`ingrediens-koeb.js`): en opslagstabel ved, hvor meget fx 1 spsk eller 1 dl af en ingrediens vejer i gram/ml, og hvordan den typisk sælges (pakke, dåse, glas, stykvis). Ingredienser i tabellen (mel, gryn, olie, sukker, krydderier, dåsevarer m.m.) lægges sammen på tværs af opskrifter i gram, uanset hvilken måleenhed hver opskrift bruger, og vises som en realistisk indkøbsmængde, fx "havregryn (min. 1 kg)" i stedet for flere linjer med "1 spsk". Ingredienser der ikke findes i tabellen (frugt, grønt, kød, fisk, mejeri, brød m.m.) vises som opskriftens egne tal (g, stk, dåse) og lægges sammen efter navn, enhed og afdeling – det er allerede praktiske indkøbsenheder. Tabellen ligger i en almindelig JS-fil og kan nemt udvides, hvis en ny opskrift bruger en ingrediens, der endnu ikke kendes.
     - Listen kan ses og ryddes for enkelte varer eller helt på Indkøb-siden.
     - *Fremtidig forbedring (kræver Køleskab)*: tjek hver ingrediens op mod Køleskab. Ingredienser der slet ikke er afkrydset i køleskabet, tilføjes direkte. For ingredienser der ER afkrydset (vi har den, men ikke nødvendigvis nok), spørger appen: "Hvor meget har du af [ingrediens] i køleskabet?" og sammenligner svaret med den mængde, opskriften kræver. Er der ikke nok, tilføjes kun den manglende mængde. Er der nok, tilføjes ingrediensen slet ikke.
6. **Køkkenvisning**: *V1 (bygget)* – åbnes via "Start køkkenvisning" på opskriftssiden (`koekkenvisning.html?id=...`). Ét skridt ad gangen: først en ingrediensoversigt, så hvert trin med meget stor tekst, til sidst en "Godt appetit"-skærm. Store Forrige/Næste-knapper fastgjort i bunden. Finder automatisk minuttal i trinteksten (fx "bag i 40 minutter") og viser en nedtællingstimer med lyd og vibration, når tiden er gået. Holder skærmen tændt med Wake Lock API, så vidt browseren understøtter det (fejler stille, hvis ikke).
7. **Online og offline**: *V1 (bygget, mangler kun selve udgivelsen)* – `manifest.json` (navn, ikon, farver, `display: standalone`) og en service worker (`sw.js`) er sat op på alle sider via `app.js`. Ikoner ligger i `icons/` (genereret som en skål+ske i appens farver, i 512/192/180/32 px samt en maskable-udgave til Android). Service workeren henter altid friskt fra nettet, når der er forbindelse, og falder tilbage til det senest gemte, hvis ikke – testet ved at slukke serveren og bekræfte, at opskrifter, ingredienser og køkkenvisning stadig virker offline.
   - **Vedligehold**: Hvis der tilføjes/omdøbes filer i appen, skal `PRECACHE_FILER`-listen i `sw.js` opdateres, og `CACHE_NAVN` bumpes (fx v2 → v3), ellers bliver ændringerne ikke hentet af browsere, der allerede har installeret appen.
   - **Mangler**: selve udgivelsen på GitHub Pages (Settings → Pages → Deploy from branch `main`) og at teste "Føj til hjemmeskærm" på en rigtig iPad.
